...
//Функция ищет из суммы каких элементов состоит число N, так как ищем по порядку от меньшего, то первый найденный будет удовлетворять условию задачи.
...
//Функция возвращает хэш список элементов состоящий из простых чисел от 2 до N
HashSet<int> get_hash_prime_numbers(int N)
{
    ...
    for(int i=3; i < N; i++)
    {
        ...
        if(hash.Contains(i))
        {
            //тут немного не правильно, потому что пробегаться будем по всем элементам которые остались, а нужно бы по всем которые после текущего
            foreach(int a in hash)
            {
